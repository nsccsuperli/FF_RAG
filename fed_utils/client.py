...
class GeneralClient:
    def __init__(self, client_id, model, data_path, output_dir):
        ...
        # self.local_data_path = os.path.join(data_path, "HealthCareMagic_{}.json".format(self.client_id))
    ...
    def terminate_local_training(self, epoch, local_dataset_len_dict, previously_selected_clients_set):
        ...
        single_output_dir = os.path.join(self.output_dir, str(epoch), "local_output_{}".format(self.client_id),"local_abcd")
        print("Saving adapter weights to:", single_output_dir)
        os.makedirs(single_output_dir, exist_ok=True)
        # 各个客户端训练的文件为pytorch_model.bin,全局模型分发的为adapter.bin

        torch.save(new_adapter_weight, single_output_dir + "/pytorch_model.bin")

        # Acc of Epoch 1 is: 0.2928562953808576
        #  67%|######6   | 2/3 [1:14:49<43:40, 2620.03s/it]
        # Conducting the client selection
        # important： 恢复模型到微调前状态：获取老权重并用 set_peft_model_state_dict 写回
    ...
    def terminate_local_training1(self, epoch, local_dataset_len_dict, previously_selected_clients_set):
        ...
        return self.model, local_dataset_len_dict, previously_selected_clients_set, last_client_id
    def terminate_local_training2(self, epoch, local_dataset_len_dict, previously_selected_clients_set):
