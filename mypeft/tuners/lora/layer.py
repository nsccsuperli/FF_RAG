...
from mypeft.tuners.tuners_utils import BaseTunerLayer, check_adapters_to_merge
from mypeft.utils.integrations import gather_params_ctx
from mypeft.utils.other import transpose
...
class LoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "lora_C", "lora_D", "lora_embedding_A", "lora_embedding_B")
    ...
    def update_layer(
        ...
        self.lora_C[adapter_name] = nn.Linear(self.out_features, self.global_r, bias=False)
        self.lora_D[adapter_name] = nn.Linear(self.global_r, self.out_features, bias=False)
        if use_rslora:
    ...
    def loftq_init(self, adapter_name):
        from mypeft.utils.loftq_utils import loftq_init
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    # 加入 C 和 D 层： A → B → C → D
                    lora_output_A = lora_A(dropout(x))
                    # print(f"lora_output_A shape: {lora_output_A.shape}")
                    lora_output_B = lora_B(lora_output_A)
                    # print(f"lora_output_B shape: {lora_output_B.shape}")
                    lora_output_C = lora_C(lora_output_B)
                    lora_output_D = lora_D(lora_output_C)
                    result = result + lora_output_D * scaling
                else:
                    x = dropout(x)
